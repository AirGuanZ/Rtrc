...
RC<Shader> Compiler::Compile(
    ...
    bool                            debug) const
{
    for(size_t i = 0; i < shader.source.size() && shader.source[i] != '\n'; ++i)
    {
        if(!std::isspace(shader.source[i]))
        {
            throw Exception(fmt::format("First line of shader {} must be empty", shader.name));
        }
    }
    ...
    for(auto &&[groupIndex, group] : Enumerate(shader.bindingGroups))
    {
        ...
        if(!group.uniformPropertyDefinitions.empty() && !group.bindings.empty())
        {
    ...
    macros["rtrc_sampler(NANE, ...)"]                = "_rtrc_sampler_##NAME";
    ...
    for(auto &group : shader.bindingGroups)
    {
        ...
        for(auto &&[bindingIndex, binding] : Enumerate(group.bindings))
        {
            ...
            else
            {
                ...
                // The actual resource definition is moved to shader beginning (see _rtrc_generated_shader_prefix)
        ...
        macros.insert({ std::move(groupLeft), std::move(groupRight) });
    ...
    shaderInfo.source         = "_rtrc_generated_shader_prefix" + shader.source;
