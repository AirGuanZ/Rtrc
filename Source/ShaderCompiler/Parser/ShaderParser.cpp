...
namespace ShaderParserDetail
{
    ...
    void ParseEntries(const std::string &source, ParsedShaderVariant &variant)
    {
        size_t beginPos = 0;
        while(true)
        {
            ...
            const size_t pos = FindFirstKeyword(source, KEYWORDS, beginPos, keyword);
        ...
        const std::string              &source)
    {
        ...
        macros["RTRC_SHADER_PREFIX_MACRO"] = {};
        ...
        RemoveCommentsAndStrings(preprocessedSource);

        // Parse entries

        ParseEntries(preprocessedSource, variant);

        // Parse bindings

        std::vector<ParsedBinding> allBindings;
        ParseBindings(preprocessedSource, allBindings);
        ParseBindingGroups(preprocessedSource, allBindings, variant.ungroupedBindings, variant.bindingGroups);
        ParseBindingAliases(preprocessedSource, variant.aliases);
        ParseInlineSamplers(preprocessedSource, variant.inlineSamplerDescs, variant.inlineSamplerNameToDesc);
        ParsePushConstantRanges(preprocessedSource, variant.pushConstantRanges);
    }
...
ParsedShader ParseShader(const ShaderCompileEnvironment &envir, DXC *dxc, const RawShader &rawShader)
{
    ...
    source.insert(rawShader.charBegin, "RTRC_SHADER_PREFIX_MACRO");
    ...
    ParsedShader ret;
    ret.name           = rawShader.shaderName;
    ret.sourceFilename = rawShader.filename;
