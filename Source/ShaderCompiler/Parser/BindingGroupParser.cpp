...
void ParseBindings(const std::string &source, std::vector<ParsedBinding> &bindings)
{
    bindings.clear();
    ...
    while(true)
    {
        ...
        const size_t pos = FindFirstKeyword(source, KEYWORDS, keywordBeginPos, keyword);
    ...
    std::vector<ParsedBindingGroup>  &groups)
{
    ...
    while(true)
    {
        ...
        std::string_view keyword;
        const size_t groupPos = FindFirstKeyword(source, KEYWORDS, keywordBeginPos, keyword);
        ...
        group.uniformStages = groupStages;
        tokens.ConsumeOrThrow(")");
        tokens.ConsumeOrThrow("{");
    ...
    std::vector<ParsedBindingAlias> &aliases)
{
    ...
    while(true)
    {
        const size_t pos = FindKeyword(source, "rtrc_alias", keywordBeginPos);
        if(pos == std::string::npos)
        {
            break;
        }
    ...
    std::map<std::string, int>     nameToDescIndex)
{
    ...
    while(true)
    {
        const size_t pos = FindKeyword(source, "rtrc_sampler", keywordBeginPos);
        if(pos == std::string::npos)
        {
            break;
        }
    ...
    std::vector<ParsedPushConstantRange> &ranges)
{
    ...
    while(true)
    {
        const size_t pos = FindKeyword(source, "rtrc_push_constant", keywordBeginPos);
        if(pos == std::string::npos)
        {
            break;
        }
