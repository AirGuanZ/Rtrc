...
#include <Rtrc/Utils/MacroOverloading.h>
...
namespace BindingGroupDSL
{
    ...
    struct MemberProxy_ConstantBuffer : T
    {
        ...
        using Struct = T;
        RC<ConstantBuffer> _rtrcObj;

        auto &operator=(RC<ConstantBuffer> value)
        {
            _rtrcObj = std::move(value);
            return *this;
        }
...
template<BindingGroupDSL::RtrcGroupStruct T>
void ApplyBindingGroup(ConstantBufferManager *cbMgr, BindingGroup &group, const T &value)
{
    int index = 0;
    T::ForEachMember([&]<typename M>(M T::* ptr, const char *name, RHI::ShaderStageFlag stages)
    {
        ...
        if constexpr(MemberElement::BindingType == RHI::BindingType::ConstantBuffer)
        {
            ...
            else
            {
                ...
                auto cbuffer = cbMgr->Create();
                cbuffer->SetData(static_cast<const CBufferStruct &>(value.*ptr));
...
template<BindingGroupDSL::RtrcGroupStruct T>
void ApplyBindingGroup(ConstantBufferManager *cbMgr, const RC<BindingGroup> &group, const T &value)
{
...
void BindingGroup::Set(const T &value)
{
    ...
    Rtrc::ApplyBindingGroup(manager_->_rtrcGetConstantBufferManager(), *this, value);
}
