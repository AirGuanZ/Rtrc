...
namespace
{
    ...
    bool IsSuitable(VkPhysicalDevice device, const DeviceDescription &desc)
    {
        // api version
        ...
        if(!timelineSemaphoreFeatures.timelineSemaphore || !sync2Features.synchronization2)
        {
            return false;
        }
...
VulkanPhysicalDevice VulkanPhysicalDevice::Select(VkInstance instance, const DeviceDescription &desc)
{
...
std::vector<const char*> VulkanPhysicalDevice::GetRequiredExtensions(const DeviceDescription &desc)
{
