...
/*
 * Modified by AirGuanZ, 2024.3.3
 * Add ConcurrentLRUCache::find_or_insert to allow efficient insertion when not found
 */
...
public:
  ...
  /**
   * Try to find a value by key, and call accessFunc with the result it found.
   * If not found, createFunc will be called to fetch a value. The value is inserted and
   * accessFunc will be called with the inserted value.
   *
   * Returns true if actually inserted
   */
  template<typename AccessFunc, typename CreateFunc>
  bool find_or_insert(const TKey& key, const AccessFunc& accessFunc, const CreateFunc& createFunc);
...
private:
  /**
   * Move an existing node to the front of the list with mutex protection
   */
  void tryMoveFront(ListNode* node);

  void onInsertion(ListNode *node);
...
find(ConstAccessor& ac, const TKey& key) {
  ...
  tryMoveFront(hashAccessor->second.m_listNode);
  return true;
}
...
template <class TKey, class TValue, class THash>
template <typename AccessFunc, typename CreateFunc>
bool ConcurrentLRUCache<TKey, TValue, THash>::find_or_insert(
    const TKey &key, const AccessFunc &accessFunc, const CreateFunc &createFunc)
{
  if(HashMapConstAccessor hashAccessor; m_map.find(hashAccessor, key)) {
    tryMoveFront(hashAccessor->second.m_listNode);
    accessFunc(hashAccessor->second.m_value);
    return false;
  }
  ...
  bool inserted = false;
  HashMapAccessor hashAccessor;
  if(m_map.insert(hashAccessor, key)) {
    ListNode *node = new ListNode(key);
    HashMapValue &value = hashAccessor->second;
    value.m_listNode = node;
    value.m_value = createFunc();
    onInsertion(node);
    inserted = true;
  }
  else
...
template <class TKey, class TValue, class THash>
void ConcurrentLRUCache<TKey, TValue, THash>::tryMoveFront(ListNode* node)
{
...
template <class TKey, class TValue, class THash>
void ConcurrentLRUCache<TKey, TValue, THash>::onInsertion(ListNode* node)
{
